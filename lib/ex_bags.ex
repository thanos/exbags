defmodule ExBags do
  ...
  ExBags provides a duplicate bag implementation that allows multiple values for the same key.
  Values are stored as lists, enabling tracking of multiple occurrences for each key.

  ## Core Operations

  - `new/0` - Creates a new empty bag
  - `put/3` - Adds a value to the bag for the given key
  - `get/2` - Gets all values for a given key from the bag
  - `keys/1` - Gets all keys from the bag
  - `values/1` - Gets all values from the bag, flattened into a single list
  - `update/3` - Updates the values for a key in the bag using a function

  ## Set Operations

  - `intersect/2` - Returns key-value pairs that exist in both bags (as tuples)
  - `difference/2` - Returns key-value pairs that exist in the first bag but not in the second
  - `symmetric_difference/2` - Returns key-value pairs that exist in either bag but not in both
  - `reconcile/2` - Performs reconciliation similar to SQL FULL OUTER JOIN
  ...
  For detailed documentation, examples, and performance benchmarks, see the [README](README.md).
  """
